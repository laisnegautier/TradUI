...
class Translator extends Component {
  constructor(props) {
    super(props);
    this.state = { texte: "", langageDetecte: "attendre", langageDeTraduction: "en", traduction: "..." };
  ...
  ecouterTraduction = () => {
    Speech.speak(this.state.traduction, { language: this.state.langageDeTraduction });
  }

  _identifierLangue = () => {
    fetch("https://api.eu-gb.language-translator.watson.cloud.ibm.com/instances/e3752ef5-06ea-4b46-9d33-6142a579f9d8/v3/identify?version=2018-05-01", {
      body: this.state.texte,
      headers: {
        Authorization: "Basic YXBpa2V5OlhOVE1qVEZvam4tNmw1b2dLaWo2OEtiMkcxOGo3QTFFejJrT3lYakZTV3Nh",
        "Content-Type": "text/plain"
      },
      method: "POST"
    }).then(response => response.json())
      // .then(responseJson => console.log(responseJson))
      .then(jsonResponse => { this.setState({ langageDetecte: jsonResponse.languages[0].language }); });
  }

  _traduire = () => {
    fetch("https://api.eu-gb.language-translator.watson.cloud.ibm.com/instances/e3752ef5-06ea-4b46-9d33-6142a579f9d8/v3/translate?version=2018-05-01", {
      body: JSON.stringify({
        text: this.state.texte,
        model_id: this.state.langageDetecte + "-" + this.state.langageDeTraduction
      }),
      ...
      .then(responseJson => { this.setState({ traduction: responseJson.translations[0].translation }) });
    // .then(responseJson => console.log(responseJson));
  }
  ...
  render() {
    return <View>
      <TextInput
        style={{ padding: 20, borderWidth: 0, marginVertical: 20, marginHorizontal: 10, elevation: 3 }}
      ...
      <Button title="Ecouter Texte Initial" onPress={this.ecouterTexteInitial} />
